...
const Slicers = () => {
  ...
  useEffect(() => {
    document.title = "Слайсеры для нарезки мяса — купить от производителя";
    ...
    setMeta("description", "Слайсеры для нарезки мяса, колбас и деликатесов на порции и ломтики. Купить от производителя с гарантией. Доставка и установка по всей России. Поможем с выбором.");
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <SiteHeader current="/slicers" onGetKp={() => { setModalProduct(""); setModalOpen(true); }} />
                    ...
                    return (
                      <div key={item.id} id={`product-${item.id}`} className="bg-white border border-border rounded-2xl overflow-hidden shadow-sm hover:shadow-xl hover:border-primary/40 transition-all flex flex-col group">
                        ...
                        <div className="p-5 flex flex-col flex-1">
                          <h3 className="font-bold text-2xl text-foreground mb-2 leading-snug cursor-pointer hover:text-primary transition-colors" onClick={() => navigate(productPath("slicers", item))}>{item.name}</h3>
                          ...
                          <div className="flex flex-col gap-2 mt-2">
                            ...
                            <div className="flex gap-2">
                              <button onClick={() => navigate(productPath("slicers", item))} className="flex-1 py-3.5 border-2 border-orange-500 text-orange-500 rounded-xl text-base font-semibold hover:bg-orange-50 transition-all">Подробнее</button>
