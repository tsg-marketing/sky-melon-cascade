                        ...
                        <div className="p-5 flex flex-col flex-1">
                          ...
                          <div className="flex flex-col gap-2 mt-2">
                            ...
                            <div className="flex gap-2">
                              <button onClick={() => navigate(productPath("injector", item))} className="flex-1 py-3.5 border-2 border-orange-500 text-orange-500 rounded-xl text-base font-semibold hover:bg-orange-50 transition-all">Подробнее</button>
            ...
            <div className="overflow-y-auto" style={{ maxHeight: "95dvh" }}>
              <div className="flex flex-col sm:flex-row gap-0">
                <div className="bg-gray-50 p-4 sm:p-6 flex flex-col gap-3 sm:w-80 flex-shrink-0">
                  ...
                  {selectedItem.pictures.length > 1 && (<div className="flex gap-2 overflow-x-auto pb-1">{selectedItem.pictures.map((pic, pi) => (<button key={pi} onClick={() => setSelectedSlide(pi)} className={`flex-shrink-0 w-12 h-12 rounded-lg overflow-hidden border-2 transition-all ${pi === selectedSlide ? "border-primary shadow-md" : "border-transparent opacity-60 hover:opacity-100"}`}><img src={pic} alt="" referrerPolicy="no-referrer" className="w-full h-full object-contain bg-white p-1" /></button>))}</div>)}
                </div>
              ...
              <div className="px-5 sm:px-6 pb-6 flex flex-col gap-4 border-t border-border/40">
                {selectedItem.all_params.length > 0 && (<div className="pt-4"><p className="text-xs font-semibold text-muted-foreground uppercase tracking-widest mb-2">Характеристики</p><div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8">{selectedItem.all_params.filter((p) => p.name !== "GUID").map((p, pi) => (<div key={pi} className="flex justify-between gap-4 py-1.5 border-b border-border/40 text-sm"><span className="text-muted-foreground">{p.name}</span><span className="font-medium text-foreground text-right">{p.value}</span></div>))}</div></div>)}
                {selectedItem.description && (<div><p className="text-xs font-semibold text-muted-foreground uppercase tracking-widest mb-2">Описание</p><div className="text-sm text-muted-foreground leading-relaxed [&_ul]:list-disc [&_ul]:pl-5 [&_ol]:list-decimal [&_ol]:pl-5 [&_li]:my-1 [&_p]:my-2 [&_strong]:font-semibold [&_strong]:text-foreground [&_a]:text-primary [&_a]:underline" dangerouslySetInnerHTML={{ __html: selectedItem.description }} /></div>)}
            ...
            <div className={`transition-all duration-1000 delay-300 ${vis("contacts") ? "opacity-100 translate-x-0" : "opacity-0 translate-x-8"}`}>
              <div className="p-8 bg-background border-2 border-primary/15 rounded-3xl shadow-sm">
                ...
                <div className="space-y-4">
                  ...
                  <button onClick={() => { if (contactsName.trim() && isValidPhone(contactsPhone) && contactsConsent && !sending) { sendLead({ name: contactsName, phone: contactsPhone, comment: contactsComment, topic: 'инъекторы для мяса', formType: 'contacts' }); setContactsName(""); setContactsPhone(""); setContactsComment(""); setContactsPhoneTouched(false); setContactsConsent(false); } }} disabled={!contactsName.trim() || !isValidPhone(contactsPhone) || !contactsConsent || sending} style={{ backgroundColor: "#D98E5C" }} className="w-full py-4 text-white rounded-xl font-bold text-base hover:brightness-95 transition-all shadow-sm disabled:opacity-40">{sending ? "Отправляем..." : "Отправить"}</button>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm" onClick={() => setModalOpen(false)}>
          <div className="bg-white rounded-3xl shadow-2xl p-8 w-full max-w-md" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              ...
              <button onClick={() => { if (modalName.trim() && isValidPhone(modalPhone) && modalConsent && !sending) { sendLead({ name: modalName, phone: modalPhone, product: modalProduct, topic: 'инъекторы для мяса', formType: 'modal' }); setModalOpen(false); setModalName(""); setModalPhone(""); setModalPhoneTouched(false); setModalConsent(false); } }} disabled={!modalName.trim() || !isValidPhone(modalPhone) || !modalConsent || sending} style={{ backgroundColor: "#D98E5C" }} className="w-full py-4 text-white rounded-xl font-bold text-base hover:brightness-95 transition-all shadow-sm disabled:opacity-40">{sending ? "Отправляем..." : "Отправить"}</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid md:grid-cols-4 gap-8 mb-10">
            ...
            <div>
              ...
              <div className="space-y-2">
                {["#benefits", "#compare", "#service", "#technosib", "#faq"].map((href, i) => (<a key={i} href={href} className="block text-sm text-muted-foreground hover:text-primary transition-colors">{["Преимущества", "Почему работает лучше", "От подбора до запуска", "О компании Техно-Сиб", "Вопросы"][i]}</a>))}
              </div>
            </div>
            <div>
